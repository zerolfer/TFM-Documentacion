...
%    \renewcommand{\tableshortname}{Tabla}
    ...
    %	RESUMEN
    \section*{RESUMEN}
    En este TFM se resuelve una nueva variación del problema de asignación de controladores aéreos a sus puestos de trabajo que precisa de la replanificación de una solución anterior para poder resolver una serie de contingencias inesperadas, teniendo que modificar parte de la planificación de la forma más optima posible. Para resolver este problema, se propone una metodología dividida en dos fases, una de inicialización y otra de optimización mediante la metaheurística \vns{}~(VNS). Una vez descrito tanto el dominio del problema como la metodología propuesta, se ofrecen detalles específicos de la implementación así como el proceso de ajuste de los parámetros del sistema. Finalmente se realiza una comparativa del sistema construido cambiando únicamente la la metaheurística empleada en la segunda fase, utilizando el llamado \sa{}, Recocido Simulado, que es aquella empleada en proyectos previos a este TFM.


    %	SUMMARY
    \section*{SUMMARY}
    This Master's Final Thesis solves a new variation of the air traffic controller work-shift scheduling problem, which requires the re-planning of a previous solution in order to solve a set of unexpected contingencies that requires modifying part of the planning in the most optimal way as possible. To solve this problem, we propose a methodology divided into two phases, one for initialization and another for optimization using the metaheuristic called \vns {}~(VNS). Once both the problem domain and the proposed methodology have been described, specific details of the implementation are provided, as well as the process of tunning the system parameters. Finally, a comparison of the built system is made, changing only the metaheuristic used in the second phase, using the so-called \sa{}~(SA), which is the one used in previous projects to this TFM.
...
%	\graphicspath{{anexos/recursos/}}
    ...
    \include{anexos/AnexoA-Formato-Planificacion-Inicial/AnexoA}

%\blankpage

 	\include{anexos/AnexoB-Diagrama-Clases-Completo/AnexoB}
	\blankpage

 	\include{anexos/AnexoC-Flame-Diagram/AnexoC}
	\blankpage
	
 	\include{anexos/AnexoD-Tabla-Casos/AnexoD}
	\blankpage
	
 	\include{anexos/AnexoE-Ejemplo-Ajuste-Parametrico/AnexoE}
%	\blankpage
%	
% 	\include{anexos/AnexoF-Recopilacion-Soluciones-Por-Fases/AnexoF}
