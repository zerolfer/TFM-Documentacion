...
\begin{enumerate}[label=\alph*.]
\item El tipo de VNS: VND, BVNS, RVNS...
\item El orden de los entornos:hacia adelante, hacia atrás, etc.
\item La forma de elegir los entornos: determinista, probabilista
\end{enumerate}

Vamos a analizarlos por separado en los siguientes párrafos.

El tipo de VNS es la forma más clara y sencilla de ajustar la capacidad explotatoria/explotatoria de la búsqueda. Si partimos de la definición básica del VNS, el \textit{Basic VNS} (recuérdese mediante el \autoref{algoritmo:Basic-VNS} a principio del capítulo)
