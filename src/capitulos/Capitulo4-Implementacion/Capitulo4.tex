...
La mayor parte de las pruebas realizadas al sistema fueron manuales, observando el fichero excel de output y analizando el comportamiento del algoritmo y detectar así anomalías en él.

Por otro lado, se llevaron a cabo test unitarios para testar la funcionalidad de métodos concretos, sobretodo debido a la dificultad de operar a nivel de carácter. 
El primero es \texttt{getIntervalos(turno:String)} que se

Respecto a test de integración o carga el sistema no querería de ellos. Sin embargo, y ya fuera del alance definido del proyecto, cuando se produzca el despliegue real de la aplicación en los sistemas aeroportuarios, sí serán requeridas.
