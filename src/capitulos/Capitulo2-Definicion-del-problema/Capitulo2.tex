...
Pues diremos que dos sectores \textit{son afines entre sí} si comparten al menos un volumen entre ellos. Para saber si dos sectores son afines, utilizaremos la llamada \textit{matriz de afinidad} que, si bien puede ser construida manualmente, en nuestro caso será tratada como un input del sistema para ahorrar tiempo innecesario de cómputo, pues es constante para cada dependencia (en el \autoref{Anexo:formato-planificacion-inicial} se muestra un ejemplo). %TODO revisar que efectivamente lo hay!!!
